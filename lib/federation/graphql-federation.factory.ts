...
export class GraphQLFederationFactory {
  ...
  private overrideFederatedResolveType(
    ...
    //
    //   TODO: Fix the type error on `resolveType` by forcing a string return type, or allow object return type by annotating resolveType differently (if possible/preferable). For now I'll just trick the type checker by returning a string type.
    //
    //   Type '(value: unknown, context: unknown, info: GraphQLResolveInfo, abstractType: GraphQLAbstractType) => Promise<string | GraphQLObjectType<any, any>>' is not assignable to type 'GraphQLTypeResolver<any, any>'.
    //   Type 'Promise<string | GraphQLObjectType<any, any>>' is not assignable to type 'PromiseOrValue<string>'.
    //     Type 'Promise<string | GraphQLObjectType<any, any>>' is not assignable to type 'Promise<string>'.
    //       Type 'string | GraphQLObjectType<any, any>' is not assignable to type 'string'.
    //         Type 'GraphQLObjectType<any, any>' is not assignable to type 'string'.ts(2322)
    //
    ...
    typeInFederatedSchema.resolveType = async (
      ...
      const resultFromAutogenSchemaObject =
        resultFromAutogenSchema as GraphQLObjectType;
